...
```{r testcorrelation,message=FALSE, warning=FALSE}
...
```{r data_prep_6,message=FALSE, warning=FALSE}
summary(df)
#colSums(is.na(df))
head(df)
...
```{r prep_train_data,message=FALSE, warning=FALSE}
...
plot(cvfit)
cvfit$lambda.min   #best λ
cvfit$lambda.1se   #stable λ

```

left → lambda.min（λ with the smallest error）
right → lambda.1se（easiest and stable λ）
...
summary(lm_fit)
car::vif(lm_fit)#whether there is a multicollinearity
...
```{r lm_pic,message=FALSE, warning=FALSE}
par(mfrow = c(2, 2)) 
plot(lm_fit)              
par(mfrow = c(1, 1)) 
...
```{r lm_pic2,message=FALSE, warning=FALSE}
